...
popular method for examining the topography of an area. These profiles
represent generalized elevation profiles of a landscape section.

[geoprofiler](https://tobiste.github.io/geoprofiler/) calculates the
elevation profile along a straight line (profile line) and provides
statistical parameters (e.g. mean, standard deviation, …) for the
...
![](B_Swath_files/figure-html/load_data-1.png)
...
![](B_Swath_files/figure-html/azimuth-1.png)
...
2.  Calculate some statistics within the swath using
    [`swath_stats()`](https://tobiste.github.io/geoprofiler/reference/swath_stats.md).

The function
[`swath_profile()`](https://tobiste.github.io/geoprofiler/reference/swath_profile.md)
requires the profile line, the raster file, and amount of
equally-spaced, parallel lines on either side of the profile line
(argument `k`) and the distance between these lines (`dist` distance
(same units as the coordinate reference system).

Here, we want to have `k=10` lines on both sides of the profile, spaced
by `dist=300` meters:
...
The output is a list that contains the extracted elevation data and also
...
[`swath_stats()`](https://tobiste.github.io/geoprofiler/reference/swath_stats.md).
We can plug in the length of the profile by specifying the parameter
...
![](B_Swath_files/figure-html/plot-1.png)
